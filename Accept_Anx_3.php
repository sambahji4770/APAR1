...
if (!empty($_GET['uid'])) {
	...
	$uid = $_GET['uid'];
	$query = "select * from user1 u1 INNER JOIN self_anx_iii r join reporting_anx_iii s JOIN reviewing_anx_iii q on u1.uid=r.uid and u1.uid=s.uid and u1.uid=q.uid WHERE u1.uid = $uid AND r.ayear=$ayear AND s.ayear=$ayear AND q.ayear=$ayear ";
